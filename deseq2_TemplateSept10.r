...
# open the tarball manually then install it
#install.packages(pkgs = "~/Documents/deg_reports/formatDESeq2", repos=NULL, type="source", dependencies = TRUE)
...
deseq2_colnames= c(geneid="EnsemblID", gene="GeneName", lfc="log2FC", pval="p", adjp="fdr"))
...
######## read TRAP DATA add new short name
# function to make the short name
ColShortName <-function (df){
  ...
  for (col in nameLs){
    ...
    nameLs[col_idx]<- paste(colstr_pts[1],colstr_pts[last_p-2],colstr_pts[last_p-1],colstr_pts[last_p],sep="_")
  }
...
####### compile a list of sample IDs and concatenate with control/ treatment asssignment
analysisWrapper<- function(
  ctrllvl,
  trtlvl,
  File_title,
  trtCol,
  ...
  #Cast this as a dataframe insted of tibble to suport the required row names
  #Change the names to the simplified names and output as counts table > includes all the data
  tempDF<-as.data.frame(rawCounts2)
  write.table(as.data.frame(tempDF),file=paste("Reps_Allcounts.txt"),sep='\t',quote=F,row.names=T)
  names(tempDF)<-labels
  write.table(as.data.frame(tempDF),file=paste("Norep_Allcounts.txt"),sep='\t',quote=F,row.names=T)
  ...
  # select the columns being used in the current expereiment
  # cast this as a dataframe insted of tibble to suport the required row names
  ...
  ############################  analyze the results and make figs
  ############################
  # bind the gene names and gene descriptions to the output

  
  ### OUTPUT REPORTS UNDER CUTOFFS 
  # full data set
  write.table(dsRe,file=paste(File_title,"Full_deseq2.txt"),sep='\t',quote=F,row.names=F)
  
  #FDR
  dsReFDR05=dim(dsRe[which(dsRe$fdr<=0.05),])
  print(paste('after filter with FDR<=.05, the count is: ',dim(dsRe[which(dsRe$fdr<=0.05),])[1]))
  write.table(dsReFDR05,file=paste(File_title,"FDR<=0.05.deseq2.txt"),sep='\t',quote=F,row.names=F)
  
  #padjBH
  dsReBH=(dsRe[which(dsRe$padj_BH<=0.05),])
  print(paste('after filter with padj_BH<=.05, the count is: ',dim(dsReBH)[1]))
  write.table(dsReBH,file=paste(File_title,"_adj_BH<=0.05.deseq2.txt"),sep='\t',quote=F,row.names=F)
  ...
  #Use gene symbol unless it is missing, then fill in with the geneID (Ensembl id)
  # Not sure that you need this >->->->->
  # >>># dsRe1[which(dsRe1$geneSymbol==""),"geneSymbol"]=dsRe1[which(dsRe1$geneSymbol==""),"GeneID"]
  write.table(dsRe1,file=paste(File_title,"_p05fc1.5_deseq2.txt"),sep='\t',quote=F,row.names=F)
  
  ##############################
  ######### Heatmap ############  -takes in norm counts from before
  normCounts<-readRDS(paste0(base_name,case.label,'vs',ctrl.label,".normalized.counts.rds"))
  
  normCounts=normCounts[dsRe$GeneID[1:100],]
  # > rownames(normCounts)=dsRe$geneSymbol[1:100]  ####NEED TO ADD GENE SYMBOL + DESCR
  
  loadPackage(c("pheatmap","gplots","RColorBrewer")) 
  #library(pheatmap)
  
  #Match the filtered column labels to the experiment type.
  # colnames(normCounts) <-labels_filt # strip the replicate information from the columns
  
  #mat_col <- data.frame(group = sample_data[colnames(normCounts),trtCol])
...
deseq2_out<-analysisWrapper("X20", "X50",File_title,trtCol,
                sampleCol,sample_data,rawCounts2, gene_names_ref, base_name)
complieTargetGene(deseq2_out, outdir, targetGeneNm1, targetGene1, File_title)

###################### EXECUTE THE RUNS #######################
#rawCounts2=HSVcounts 

File_title ="Comparison2-X50vsX20"
Basename="BaseExpt"
analysisWrapper("X20","X50",File_title,trtCol,
          sampleCol,sample_data,rawCounts2, gene_names_ref, base_name)
complieTargetGene(deseq2_out, outdir, targetGeneNm1, targetGene1, File_title)

###########
# post processing you will have to arrange files for the DEG report step
#DEG_format >
  #comp1.txt
  #comp2.txt
  #comp3.txt
...
##########
# comparing last time and this time 
# note there are 2M and 6m levels for some of these

function()

AssayPairs<-list(c("X50_sig_results.txt", "Y50_6M_results.txt"),
c("X111_sig_results.txt", "Y111_2M_results.txt"),
c("X111_sig_results.txt", "Y111_6M_results.txt"))

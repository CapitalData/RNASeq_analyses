loadPackage<-function(packageNames=NULL,update.these=F,update.all=F,
  ...
  tryCatch({
    if(install.bioc) biocLite(ask=ask,lib.loc=lib.loc)
    if(update.all) biocLite(ask=ask,lib.loc=lib.loc) #update current packages
    #if(update.all) biocLite(ask=F,siteRepos=c("http://cran.r-project.org",
    #		"http://www.bioconductor.org/packages/release/bioc")) 
    if(update.bioc) biocLite("BiocUpgrade",ask=ask,lib.loc=lib.loc) #update bioconductor
  },error=function(cond) {
...
#yx.ratio is the fig ratio, can be 'yx.ratio=2/3'
#hjust = 0.5: center the plot title
myggTheme=function(fs=15,hideLegend=F,hideLegendTitle=F,legendPosition='top',hideXlabel=F,hideYlabel=F,
                   rotateYlabel=F, rotateXtickLabel=F,rotateAngle=90, yx.ratio=NULL){
  library(ggplot2)
  if(hideLegend) legendPosition='none'
  myT=theme(axis.text=element_text(size=fs),axis.title=element_text(size=fs+3),
            plot.title=element_text(size=fs+5,hjust = 0.5),legend.title=element_text(size=fs+3),
            legend.text=element_text(size=fs),legend.position=legendPosition)
  if(hideLegendTitle) myT=myT+theme(legend.title=element_blank())
  if(hideXlabel) myT=myT+theme(axis.title.x = element_blank())
  if(hideYlabel) myT=myT+theme(axis.title.y = element_blank())
  if(rotateYlabel) myT=myT+theme(axis.title.y = element_text(angle = rotateAngle, hjust = 1))
  if(!is.null(yx.ratio)) myT=myT+theme(aspect.ratio=yx.ratio)
  if(rotateXtickLabel)  myT=myT+theme(axis.text.x = element_text(angle = rotateAngle, hjust = 1))
  
  return(myT)
}

#Remove rows or columns of 0 variance
#gep: a numberical matrix or data.frame
rmVar0=function(gep,margin=1){
  ...
  if(length(ids.var0)>0){
    if(margin==1){
      gep=gep[-ids.var0,]
    }else{gep=gep[,-ids.var0]}
  }
  
  return(gep)
}

#gep: a matrix or data frame defined by gepRowIsSample
#samps: a data frame: row: samples; col: features; rownames: sampleID
#doScale: Scale variance or not
#revPCdir: reverse the direction of PCs
do.PCA=function(gep,samps,gepRowIsSample=F,revPCdir=F,doScale=T,PCs=1:3,...){
  if(!gepRowIsSample){ gep=t(gep)}
  comSamps=intersect(rownames(gep),rownames(samps))
  if(length(comSamps)<3) stop("Less than 3 shared samps, something wrong!")
  gep=gep[comSamps,]
...
#pcaReturn: returned list from do.PCA()
#figMain: the title of the plot
#ofBaseName: the base name for output files
#classCol: the column name in sample table used for point color 
#shapeCol: the column name in sample table used for point shpae 
draw.PCA=function(pcaReturn,PC=1:3,figMain=NULL,ofBaseName=NULL,
  ...
  for(i in PC){
    ...
    }else{ figTitle=figMain}
    p=ggplot(data=pcScores,mapping=aes_string(pcx,pcy,color=classCol,shape=shapeCol))+
      geom_point(size=pointSize,alpha=8/10) +
      ggtitle(figTitle)+
      myggTheme(textSize)+ 
      xlab(paste(pcx," : ",var.perc[2,pcInd[1,i]],"%",sep=""))+
      ylab(paste(pcy," : ",var.perc[2,pcInd[2,i]],"%",sep=""))
    
    ggsave(paste(ofBaseName,".PC",pcInd[1,i],pcInd[2,i],'.png',sep=''),p,dpi=600)
  }
  
  return(p)
}

#countsTable: dataframe or matrix of gene counts. cols: sample ids ONLY; rows: gene ids.
#samCondition: a vector of sample group labels corresponding to columns of countsTable
#ctrl.label: sample group label of control; default: the 1st level in factor(samCondition)
#p.fdr: use p or fdr for DEG
#useCooksFilter: if one or more samples for a row have a distance higher than Cook's distance, 
#                the p-value for the row is set to NA; see 'cooksCutoff' for detail
#useShrunkenFC: if T, the FC isn't the ratio of case/ctrl, but the shrunken fold change
#               for the goal of reducing the FC of low expression genes, 
#               see http://genomebiology.com/content/pdf/s13059-014-0550-8.pdf
#ofBaseName: output file base name; if set, output files are saved.
#            Five output files are two volcano plot, normalized gene count, differential results for
#            all genes and DEGs only.

#Deseq2 set the FDR of some low expression genes to NA in order to increase the number
#of genes with significant FDR
deseq2=function(countsTable,samCondition,ctrl.label=NULL,case.label=NULL,
                FCs=c(1.2,1.5),p.fdr='p',ofBaseName=NULL,
                useCooksFilter=T,useShrunkenFC=T){
  ...
  dds <- DESeqDataSetFromMatrix(countData = countsTable,
                                colData = samCondition, design = ~ samCondition)
  #this is necessary because the comparison will be made to be
  #the 2nd level over the 1st level; so this is to make sure the
  #comparison is set correctly.
  dds$samCondition <- factor(dds$samCondition,
                             levels=c(ctrl.label,case.label))	
  ...
  res=res[,c("GeneID","baseMean",case.label,ctrl.label,"FC","log2FC","p","fdr")]
  if(!is.null(ofBaseName)){
    write.table(res,
                file=paste(ofBaseName,case.label,'vs',ctrl.label,"deseq2.txt",sep="."),
                sep='\t',quote=F,row.names=F)
    saveRDS(counts.norm,paste0(ofBaseName,".normalized.counts.rds"))
    volcano.plot(res$p,res$log2FC,
                 figFileName=paste(ofBaseName,case.label,'vs',ctrl.label,"volcano.png",sep="."))
    volcano.plot(res$p,res$FC,xlabStr='Fold Change',
                 figFileName=paste(ofBaseName,case.label,'vs',ctrl.label,"volcano.FC.png",sep="."))
  }
  ...
  for(FC in FCs){
    ...
    if(!is.null(ofBaseName))
      write.table(degs,
                  file=paste(ofBaseName,case.label,'vs',ctrl.label,"DEG.FC",FC,p.fdr,"0.05.txt",sep="."),
                  sep='\t',quote=F,row.names=F)
  }
...
volcano.plot=function(pvals,FC,p.cutoff=0.05,FC.cutoff=1.5, figFileName=NULL,
  ...
  p=ggplot(pointsDF,aes(x=x, y=y,  colour=as.factor(sig))) +
    ...
    geom_hline(yintercept=c(-log10(c(0.05,0.01,0.001))), linetype=2, lwd=1) + 
    geom_vline(xintercept=c(FC.cutoff,-FC.cutoff), linetype=2, lwd=1) + 
    myggTheme(20)+ggtitle(titleStr) + xlab(xlabStr) + ylab(ylabStr) +
...
RunGSEA <- function(filename, genes, gen_sym_type, pThresh, adjpThresh, MinSz, nperms){
  ...
  #print(head(fgseaRes[order(fgseaRes$pval), ]))
  ...
  #topPathwaysDown <-head(fgseaRes[order(fgseaRes$pval),][fgseaRes$ES < 0,], n=10)$pathway
  z<-fgseaRes[fgseaRes$ES < 0,]
  topPathwaysDown <-head(z[order(z$pval),]$pathway)
  
  topPathways <- c(topPathwaysUp, rev(topPathwaysDown))
  plotGseaTable(msigdb_pathways[topPathways], rank_list, fgseaRes, 
                gseaParam = 0.5)
...
Merge_Avg <- function(L_data, R_data, merge_col){
  L_data<-L_data%>%
    ...
    rename(p = p_combo) %>% rename(FC = FC_combo) %>% rename(geneSymbol=hgnc_symbol)
